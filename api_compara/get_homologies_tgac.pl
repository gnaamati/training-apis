...
my ($org1, $org2) = ('triticum_aestivum', 'triticum_iwgsc');
...
my $count = 0;
say "tgac_gene\tiwgsc_gene\t%id. tgac_cover\t%id. iwgsc_cover\tdescription";
for my $gene_id (@genes) {
    ...
    foreach my $this_homology (@{$all_homologies}) {
        ...
        foreach my $member (@{$align_members}) {
            ...
            #say $member->stable_id;   
            if ($member->genome_db_id != '2080'){
        ...
        print "$arabi_sid\t$wheat_sid\t$arabi_cover\t$wheat_cover\t$desc\n";
    }
    $count++;
    if ($count % 1000 == 0){
        warn "$count\n";
